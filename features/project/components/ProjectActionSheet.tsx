...
const ActionCard = ({ icon, title, description, onClick }: ActionCardProps) => (
  ...
  >
    ...
    <span className="text-[18px] text-[#868685] leading-none shrink-0">›</span>
  </button>
);
...
export const ProjectActionSheet = ({ onCsvImport, onCsvExport }: Props) => (
  <div className="flex flex-col gap-3.5 w-full">
    <p className="text-[13px] font-normal text-[#868685] leading-[normal]">
      案件・現場データの一括操作
    </p>
    <ActionCard
      icon="⬇"
      title="CSV登録"
      description="CSV ファイルから案件・現場を一括登録"
      onClick={onCsvImport}
    />
    <ActionCard
      icon="⬆"
      title="CSV出力"
      description="現在の案件・現場一覧を CSV でダウンロード"
      onClick={onCsvExport}
    />
  </div>
);
