...
export const TimePicker = ({
  ...
  const renderColumn = (
    ...
    <div className="flex flex-col gap-1 max-h-[200px] overflow-y-auto px-1 py-1">
      {items.map((item) => {
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      {open ? (
        ...
        >
          {renderColumn(HOURS, hh, handleHourClick)}
          <div className="w-px bg-[#e8ebe6]" />
          {renderColumn(MINUTES_5, mm, handleMinuteClick)}
        </div>
      ) : null}
    </div>
  );
